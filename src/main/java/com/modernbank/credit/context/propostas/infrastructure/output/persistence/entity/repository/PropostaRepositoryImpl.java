...
// Removido @Repository para evitar bean duplicado; usar PropostaRepositoryAdapter como
// implementação

/**
 * Implementação do repositório de Propostas usando Spring Data JPA. Adapta a interface de domínio
 * para a implementação de persistência.
 *
 * <p>Padrão: Repository Pattern + Adapter Pattern - Implementa o contrato definido em
 * PropostaRepository - Encapsula a lógica de persistência com JPA - Permite trocar implementação
 * sem afetar o domínio
 *
 * <p>Anotação @Repository: - Identifica como componente de acesso a dados do Spring - Ativa
 * tratamento automático de exceções JPA
 */
/**
 * DEPRECATED: Implementação antiga do repositório de domínio baseada diretamente no JPA repository.
 * Mantida apenas por histórico; não registrada como bean Spring. Prefira usar o adapter {@link
 * PropostaRepositoryAdapter} junto com {@link SpringDataPropostaRepository}.
 */
@Deprecated
...
public class PropostaRepositoryImpl implements PropostaRepository {
  ...
  /**
   * Salva uma proposta no banco de dados. Converte da entidade de domínio para JPA e vice-versa.
   *
  ...
  /**
   * Busca uma proposta pelo ID. Lança exceção se não encontrado (opcional).
   *
  ...
  public Optional<Proposta> buscarPorId(UUID id) {
    ...
    return jpaRepository.findById(id).map(PropostaEntity::toDomain);
  }

  /**
   * Busca uma proposta pelo ID e lança exceção se não encontrada. Util para quando a proposta DEVE
   * existir.
   *
  ...
  public Proposta buscarPorIdOuLancarExcecao(UUID id) {
    return buscarPorId(id)
        .orElseThrow(
            () -> new PropostaNotFoundException("Proposta com ID " + id + " não encontrada"));
  }
}
