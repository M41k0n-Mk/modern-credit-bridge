package com.modernbank.credit.context.propostas.infrastructure.output.messaging;
...
public class SqsPropostaAdapter implements PropostaNotifier {
  ...
  public void publicar(DomainEvent event) {
    try {
      String json;
      // Backward compatibility: para PropostaCriadaEvent manter o contrato antigo da fila (payload
      // de Proposta)
      if (event instanceof PropostaCriadaEvent e) {
        var payloadCompat = new LinkedHashMap<String, Object>();
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "[SqsPropostaAdapter] Enviando evento para SQS. queueUrl={} type={} aggregateId={}",
            queueUrl,
            event.type(),
            event.aggregateId());
      }
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "[SqsPropostaAdapter] Evento publicado na SQS com sucesso. type={} aggregateId={}",
            event.type(),
            event.aggregateId());
      }
    } catch (Exception e) {
