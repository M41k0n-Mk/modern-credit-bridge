package com.modernbank.credit.context.propostas.domain.usecase;
...
import com.modernbank.credit.context.propostas.domain.event.PropostaCriadaEvent;
...
/**
 * Use Case responsável pela criação de uma nova proposta de crédito. Orquestra a lógica de negócio
 * necessária para criar uma proposta.
 *
 * <p>PADRÃO: Use Case / Interactor (Clean Architecture) Responsabilidades: 1. Validação (delega ao
 * entity se for simples) 2. Coordenação entre agregados/services 3. Persistência dos dados
 */
public class CriarPropostaUseCase {
  ...
  /**
   * Executa a criação de uma nova proposta. Fluxo: 1. Valida a proposta (já feito no construtor da
   * Proposta) 2. Poderia chamar um DomainService se a lógica fosse complexa 3. Persiste no
   * repositório
   *
  ...
  public Proposta executar(Proposta proposta) {
    ...
    Proposta propostaParaSalvar =
        PropostaFactory.reconstruir(
            proposta.getId(), proposta.getCpf(), proposta.getValor(), status);

    Proposta salva = repository.salvar(propostaParaSalvar);

    // Notifica (ex.: publica evento de domínio)
    if (propostaNotifier != null) {
