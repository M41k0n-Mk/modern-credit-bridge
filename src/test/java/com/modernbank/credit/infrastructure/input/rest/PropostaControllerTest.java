 ...
 *
 * <p>Testa o endpoint REST utilizando MockMvc para simular requisições HTTP sem iniciar o servidor.
 *
 * @author ModernBank
 */
...
class PropostaControllerTest {
  ...
  @Mock private CriarPropostaUseCase criarPropostaUseCaseMock;
  ...
  void setUp() {
    ...
    this.mockMvc =
        MockMvcBuilders.standaloneSetup(new PropostaController(criarPropostaUseCaseMock)).build();
  }
  ...
  void deveCriarPropostaRetornando201() throws Exception {
    ...
    when(criarPropostaUseCaseMock.executar(any(Proposta.class))).thenReturn(propostaSalva);

    String requestJson = objectMapper.writeValueAsString(request);

    // Act & Assert
    mockMvc
        .perform(post("/v1/propostas").contentType(MediaType.APPLICATION_JSON).content(requestJson))
  ...
  void deveRetornarBadRequestParaJsonInvalido() throws Exception {
    ...
    mockMvc
        .perform(
            post("/v1/propostas").contentType(MediaType.APPLICATION_JSON).content(jsonInvalido))
        .andExpect(status().isBadRequest());
  }
}
