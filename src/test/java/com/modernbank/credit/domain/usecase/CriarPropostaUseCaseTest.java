 ...
 *
 * <p>Padrão: AAA (Arrange, Act, Assert) - Arrange: Preparar dados e mocks - Act: Executar a ação -
...
class CriarPropostaUseCaseTest {
  ...
  @Mock private PropostaRepository repositoryMock;

  @Mock private ClienteHistoricoService clienteHistoricoService;

  @Mock private PropostaNotifier propostaNotifier;
  ...
  void setUp() {
    this.useCase =
        new CriarPropostaUseCase(repositoryMock, clienteHistoricoService, propostaNotifier);
  }
  ...
  void deveCriarPropostaBuscandoDoRepositorio() {
    ...
    when(repositoryMock.salvar(any(Proposta.class))).thenReturn(propostaSalva);
  ...
  void deveCharmarRepositorioUmaVez() {
    ...
    Proposta propostaSalva =
        new Proposta(UUID.randomUUID(), proposta.getCpf(), proposta.getValor(), "PENDENTE");
